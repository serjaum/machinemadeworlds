...
class PublishingTests(unittest.TestCase):
    ...
    def test_draft_to_publication_updates_every_index_without_editing_templates(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            base = sum(1 for p in (ROOT / 'content/posts').glob('*.json')
                       if not json.loads(p.read_text(encoding='utf-8')).get('draft', False))
            self.assertEqual(module.build(target)['posts'], base)
            ...
            path.write_text(json.dumps(data), encoding='utf-8')
            self.assertEqual(module.build(target)['posts'], base + 1)
    ...
    def test_one_build_publishes_every_existing_article_in_index_and_feed(self):
        ...
        expected = sum(1 for p in (ROOT / 'content/posts').glob('*.json')
                       if not json.loads(p.read_text(encoding='utf-8')).get('draft', False))
        self.assertEqual(len(posts), expected, 'Preserve all existing articles')
