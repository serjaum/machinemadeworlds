...
(function () {
  ...
  var BEST_KEY = 'mmw.star-harvest.best.v1';
  var RUN_SECONDS = 90;
  ...
  var canvas = document.getElementById('star-harvest');
  ...
  document.addEventListener('keydown', function (ev) {
    ...
    keys[ev.code] = true;
    if (ev.code === 'Enter') {
      if (state === 'running') startRun('Fresh flight. Collect stars, avoid rocks.');
      else startRun();
  ...
  if (buttons.restart) buttons.restart.addEventListener('click', function () {
    startRun('Fresh flight. Collect stars, avoid rocks.');
  });
