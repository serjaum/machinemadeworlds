...
def autolink_glossary(body, links, skip_url=None):
    ...
    letters, digits, underscores or hyphens), so a term never fires
    inside a longer word, a snake_case identifier such as
    ``sentence_transformers`` or a hyphenated compound, and text inside
    existing anchors (link labels and destinations alike) is left
    untouched. Only text nodes are scanned: markup itself is split into
    tags vs text first, so an alias inside a title/alt attribute (or any
    tag) can never be linkified and corrupt the tag. Code samples are
    never linkified: segments wrapped in <pre>...</pre> or
    <code>...</code> are passed through verbatim.
    """
    if not links:
        return body
    ordered = sorted(links, key=lambda pair: (-len(pair[0]), pair[0].lower()))
    # Protect code samples first: <pre> blocks (which usually wrap <code>)
    # and standalone <code> segments are never linkified.
    segments = re.split(
        r'(<pre\b[^>]*>.*?</pre>|<code\b[^>]*>.*?</code>)',
        body, flags=re.S | re.I)
    for seg_index in range(0, len(segments), 2):
        chunk = segments[seg_index]
        parts = re.split(r'(<a\b[^>]*>.*?</a>)', chunk, flags=re.S | re.I)
        subs = {}
        ...
        for alias, url in ordered:
            if skip_url is not None and url == skip_url:
                continue
            pattern = re.compile(r'(?<![A-Za-z0-9_-])' + re.escape(alias) + r'(?![A-Za-z0-9_-])', re.I)
            for index in range(0, len(parts), 2):
                sub = subs[index]
