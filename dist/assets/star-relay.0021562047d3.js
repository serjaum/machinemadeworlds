/* Star Relay: turn-based DOM-grid puzzle. Solved path scrambled at deal. */
var StarRelay = (function () {
'use strict';
var N=1,E=2,S=4,W=8;
function rot(p,t){var r=((t%4)+4)%4,o=0,i;for(i=0;i<4;i+=1)if(p&(1<<i))o|=1<<((i+r)%4);return o;}
function rng32(seed) {
  var a = seed >>> 0;
  return function () {
...
var LEVELS=[[4,4,0,0],[4,4,1,0],[5,6,1,2],[5,6,2,3],[5,8,2,4],[5,8,3,5],[6,10,3,5],[6,10,3,6],[6,12,4,7],[6,12,4,8],[6,12,5,8],[6,14,5,9]];
var DAILY=[6,12,4,7];
function dailySeedUTC(d){d=d||new Date();return d.getUTCFullYear()*10000+(d.getUTCMonth()+1)*100+d.getUTCDate();}
function isoSeed(s){var t=String(s);return t.slice(0,4)+'-'+t.slice(4,6)+'-'+t.slice(6,8);}
...
function kindOf(p) {
  var n = 0, q = p;
  while (q) { n += q & 1; q >>= 1; }
  if (n === 2) return (p === 5 || p === 10) ? 'straight' : 'elbow';
  return 'tee';
}
function fixCost(b,u,s){var t;for(t=0;t<4;t+=1)if(rot(b,u+t)===s)return t;return 0;}
function genBoard(sp, rng) {
  var size = sp[0], row = (size - 1) >> 1;
  ...
  return { size: size, row: row, tiles: tiles, home: home, optimal: optimal,
    budget: optimal + sp[1], slack: sp[1] };
}
function generate(l, a) {
  return genBoard(LEVELS[l % LEVELS.length], rng32(0x9E37 + l * 7919 + (a || 0) * 131));
}
function generateDaily(seed, attempt) {
  if (typeof seed === 'string') {
    var p = seed.split('-');
    seed = (+p[0]) * 10000 + (+p[1]) * 100 + (+p[2]);
  }
  if (!seed) seed = dailySeedUTC();
  var g = genBoard(DAILY, rng32(0xD41E + seed * 97 + (attempt || 0) * 131));
  g.daily = true; g.seed = seed;
  return g;
}
function flow(tiles, size, row) {
  ...
  var lit = new Array(tiles.length), queue = [row * size], won = false, q = 0, i;
  for (i = 0; i < lit.length; i += 1) lit[i] = false;
  lit[row * size] = true;
  while (q < queue.length) {
    var cur = queue[q++], cr = (cur / size) >> 0, cc = cur % size, cp = portsAt(cur);
    var ns = [[cr-1,cc,N,S],[cr,cc+1,E,W],[cr+1,cc,S,N],[cr,cc-1,W,E]];
    for (i = 0; i < 4; i += 1) {
      var nr = ns[i][0], nc = ns[i][1], ni = nr * size + nc;
      if (nr < 0 || nc < 0 || nr >= size || nc >= size || lit[ni]) continue;
...
function glyphFor(kind, ports) {
  ...
  var g = { 5: '│', 10: '─', 3: '└', 6: '┌', 12: '┐', 9: '┘', 14: '┬', 11: '┴', 7: '├', 13: '┤' };
  return g[ports] || '·';
}
var api = { N: N, E: E, S: S, W: W, LEVELS: LEVELS, rot: rot,
  rng32: rng32, generate: generate, generateDaily: generateDaily,
  dailySeedUTC: dailySeedUTC, flow: flow, fixCost: fixCost,
...
function txt(sel, msg) { setText(stage.querySelector(sel), msg); }
function loadInt(k, cap) {
  try {
    var n = parseInt(window.localStorage.getItem(k), 10);
...
function tone(fr, se, ki, wh) {
  ...
  try {
    ...
    gain.gain.exponentialRampToValueAtTime(0.0001, t0 + se);
    osc.connect(gain); gain.connect(actx.destination);
    osc.start(t0); osc.stop(t0 + se + 0.02);
  } catch (e) { return; }
}
...
var CAP = LEVELS.length - 1;
var levelIdx = 0, unlocked = loadInt(PROGRESS_KEY, CAP);
...
function show(name) {
  ...
  var ps = overlayRoot.querySelectorAll('[data-overlay]'), i;
  for (i = 0; i < ps.length; i += 1) ps[i].hidden = ps[i].getAttribute('data-overlay') !== name;
  if (name) {
    var f = overlayRoot.querySelector('[data-overlay="' + name + '"] button:not([disabled])');
    if (f) f.focus();
  }
}
function lvlBtn(w,n) {
...
function grp(t) {
  ...
  h.className = 'relay-group'; h.textContent = t;
  levelList.appendChild(h);
  var w = document.createElement('div');
  w.className = 'relay-levels';
  levelList.appendChild(w);
  return w;
}
function drawLevels() {
  if (!levelList) return;
  while (levelList.firstChild) levelList.removeChild(levelList.firstChild);
  var a = grp('Relay I — Levels 1–6'), i;
  for (i = 0; i < 6; i += 1) lvlBtn(a, i);
  var b2 = grp('Relay II — Levels 7–12');
  for (i = 6; i < 12; i += 1) lvlBtn(b2, i);
  var d = grp('Daily'), db = document.createElement('button');
  db.type = 'button'; db.className = 'relay-level';
  db.setAttribute('data-action', 'daily');
  db.textContent = 'Daily — ' + isoSeed(dailySeedUTC());
  d.appendChild(db);
}
function paint(focusRC) {
  ...
  for (var r = 0; r < board.size; r += 1) {
    for (var c = 0; c < board.size; c += 1) {
      ...
      var spin = spins[r * board.size + c] || 0;
      g.style.transform = 'rotate(' + (spin * 90) + 'deg)';
      b.appendChild(g);
      if (t.fixed) b.disabled = t.wall || t.kind === 'reactor' || t.kind === 'beacon';
      if (res.lit[r * board.size + c]) b.classList.add('lit');
      ...
      b.setAttribute('aria-label', 'Row ' + (r + 1) + ' column ' + (c + 1) + ': ' +
        tileName(t) + (t.wall ? '' : res.lit[r * board.size + c] ? ', powered' : ', unpowered'));
      boardNode.appendChild(b);
    }
  }
  setText(hud.level, mode === 'd' ? 'Daily' : String(levelIdx + 1) + ' / 12');
...
function deal(ann) {
  ...
  spins = {};
  txt('[data-overlay="complete"] h3', mode === 'd' ? 'Daily relay linked' : 'Relay linked');
  drawLevels();
  show(null);
  paint(null);
  announce(ann);
...
function startLevel(n, attempt) {
  ...
  deal('Level ' + (n + 1) + ': connect the reactor to the beacon. ' + board.budget + ' moves.');
}
function startDaily(attempt) {
  mode = 'd'; tries = attempt || 0;
  var seed = dailySeedUTC();
  dailyTag = isoSeed(seed);
  board = generateDaily(seed, tries);
  deal('Daily ' + dailyTag + ': connect the reactor to the beacon. ' + board.budget + ' moves.');
}
function turn(r, c) {
  ...
  if (t.fixed) return;
  t.rot = (t.rot + 1) % 4;
  spins[r * board.size + c] = (spins[r * board.size + c] || 0) + 1;
  movesLeft -= 1;
  sfxTurn();
  var res = flow(board.tiles, board.size, board.row);
  if (res.won) {
    if (mode === 'd') {
      var msg = 'Daily ' + dailyTag + ' linked with ' + movesLeft + ' moves to spare.';
      paint([r, c]);
      txt('[data-complete-text]', msg);
      show('complete');
      announce(msg);
      sfxWin();
      return;
    ...
    if (levelIdx < CAP) {
      ...
      paint([r, c]);
      txt('[data-complete-text]', 'Level ' + (levelIdx + 1) +
        ' linked with ' + movesLeft + ' moves to spare. Plus ' + gained + ' points.');
      show('complete');
      announce('Level ' + (levelIdx + 1) + ' complete. Plus ' + gained + ' points.');
    } else {
      if (CAP > unlocked) { unlocked = CAP; saveInt(PROGRESS_KEY, CAP); }
      paint([r, c]);
      txt('[data-campaign-text]', 'All twelve relays hum. Final score ' + banked + '.');
      show('campaign');
  ...
  if (movesLeft <= 0) {
    paint([r, c]);
    show('stuck');
    announce('Out of moves on ' + (mode === 'd' ? 'daily ' + dailyTag : 'level ' + (levelIdx + 1)) + '. Retry for a fresh scramble.');
    sfxLose();
    return;
  }
  paint([r, c]);
...
function doAction(name) {
  sfxPick();
  if (name === 'start') startLevel(Math.min(unlocked, CAP), 0);
  else if (name === 'daily') startDaily(0);
  ...
  else if (name === 'retry') { if (mode === 'd') startDaily(tries + 1); else startLevel(levelIdx, tries + 1); }
  else if (name === 'next') {
    if (mode === 'd') { drawLevels(); show('levels'); }
    else startLevel(Math.min(levelIdx + 1, CAP), 0);
  }
    ...
    if (m) { m.textContent = muted ? 'Unmute' : 'Mute'; m.setAttribute('aria-pressed', String(muted)); }
  }
  else if (name === 'motion') {
    var still = stage.classList.toggle('relay-still');
    if (motionBtn) { motionBtn.textContent = still ? 'Motion off' : 'Motion on'; motionBtn.setAttribute('aria-pressed', String(still)); }
  }
}
stage.addEventListener('click', function (ev) {
  ...
  while (el && el !== stage) {
    ...
    if (el.getAttribute && el.getAttribute('data-level')) { startLevel(+el.getAttribute('data-level') - 1, 0); return; }
    if (el.getAttribute && el.getAttribute('data-r')) { turn(+el.getAttribute('data-r'), +el.getAttribute('data-c')); return; }
    el = el.parentNode;
  }
});
document.addEventListener('keydown', function (ev) {
  ...
  if (code === 'KeyR' && onTile) {
    turn(+active.getAttribute('data-r'), +active.getAttribute('data-c'));
    return;
  }
  if (!onTile || shown) return;
  var r = +active.getAttribute('data-r'), c = +active.getAttribute('data-c');
...
if (motionBtn) {
  var s0 = stage.classList.contains('relay-still');
  motionBtn.textContent = s0 ? 'Motion off' : 'Motion on';
  motionBtn.setAttribute('aria-pressed', String(s0));
}
drawLevels();
paint(null);
show('start');
