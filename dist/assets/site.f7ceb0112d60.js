...
(() => {
  ...
  /* Scrollspy TOC + reading progress. */
  (function(){var t=document.querySelector(".toc"),b=document.querySelector(".reading-progress"),d=document.querySelector(".article-body");if(!t||!b||!d)return;var l=[].slice.call(t.querySelectorAll('a[href^="#"]'));if(!l.length)return;var m={};l.forEach(function(a){m[a.getAttribute("href").slice(1)]=a});var h=[].slice.call(d.querySelectorAll("h2[id]")).filter(function(a){return m[a.id]});if(!h.length)return;b.hidden=false;var r=window.requestAnimationFrame||function(c){c()};function s(i){l.forEach(function(a){if(a.getAttribute("href")==="#"+i)a.setAttribute("aria-current","true");else a.removeAttribute("aria-current")})}function c(){var e=document.documentElement;if(scrollY+innerHeight>=e.scrollHeight-2)return h[h.length-1].id;var n=scrollY+Math.min(innerHeight*.3,240),o=h[0].id;for(var i=0;i<h.length;i++){if(h[i].getBoundingClientRect().top+scrollY<=n)o=h[i].id;else break}return o}function p(){var r=d.getBoundingClientRect(),o=r.height-innerHeight,v=o>0?Math.min(Math.max(-r.top,0),o)/o:-r.top>=0?1:0;b.style.setProperty("--progress",String(Math.min(Math.max(v,0),1)))}function f(){s(c());p()}var k=false;function u(){if(k)return;k=true;r(function(){k=false;f()})}if("IntersectionObserver"in window){var o=new IntersectionObserver(function(){u()},{rootMargin:"-20% 0px -65% 0px"});h.forEach(function(a){o.observe(a)})}addEventListener("scroll",u,{passive:true});addEventListener("resize",u,{passive:true});f()})();
